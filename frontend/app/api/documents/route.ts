...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    if (!upstreamResponse.ok) {
      try {
        ...
        return toErrorResponse(
          parsed.message ?? parsed.detail ?? "Documents proxy request failed",
          upstreamResponse.status,
        );
      } catch {
        return toErrorResponse(
          responseBody || "Documents proxy request failed",
          upstreamResponse.status,
        );
      }
    }
    ...
    const message =
      error instanceof Error ? error.message : "Unexpected documents proxy error";
